...
export function createCreateEventOperation(
  ...
  return {
    ...
    validateInput(input: Record<string, unknown>): string[] {
      ...
      if (typeof input['summary'] !== 'string' || (input['summary'] as string).length === 0) {
        errors.push('summary is required and must be a non-empty string');
      }
      validateEventTime(input['start'], 'start', errors);
      validateEventTime(input['end'], 'end', errors);
    ...
    async execute(
      ...
      const typedInput = input as unknown as CreateEventInput;
