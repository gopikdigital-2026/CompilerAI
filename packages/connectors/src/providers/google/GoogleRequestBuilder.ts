...
export class GoogleRequestBuilder {
  ...
  constructor(
    private readonly baseUrl: string = 'https://www.googleapis.com',
  ) {
  ...
  static get(path: string, baseUrl?: string): GoogleRequestBuilder {
    return new GoogleRequestBuilder(baseUrl).setMethod('GET').addPath(path);
  }

  static post(path: string, baseUrl?: string): GoogleRequestBuilder {
    return new GoogleRequestBuilder(baseUrl).setMethod('POST').addPath(path);
  }

  static patch(path: string, baseUrl?: string): GoogleRequestBuilder {
    return new GoogleRequestBuilder(baseUrl).setMethod('PATCH').addPath(path);
  }

  static delete(path: string, baseUrl?: string): GoogleRequestBuilder {
    return new GoogleRequestBuilder(baseUrl).setMethod('DELETE').addPath(path);
  }

  setMethod(method: 'GET' | 'POST' | 'PATCH' | 'DELETE'): this {
    this.method = method;
    return this;
  }
  ...
  build(): { url: string; method: 'GET' | 'POST' | 'PATCH' | 'DELETE'; body: Record<string, unknown> | null } {
